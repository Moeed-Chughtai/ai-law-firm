...
- Conflict Check: ${matter.conflictCheck?.cleared ? '✅ Cleared' : '⚠️ Flagged'}
...
Make sure EVERY section of the document gets an annotation (either an issue or a clean bill of health).`,
    ...
    // 3. Engagement Letter — standard law firm engagement confirmation
    callLLM(
...
${matter.guardrails?.escalationRequired ? '- ⚠️ This analysis has been flagged for mandatory human attorney review before reliance' : ''}
...
| Adversarial Loop Count | ${matter.adversarialLoopCount || 0} | ${(matter.adversarialLoopCount || 0) > 0 ? '⚠️' : '✅'} |
| Conflict Check | ${matter.conflictCheck?.cleared ? 'Cleared' : 'Flagged'} | ${matter.conflictCheck?.cleared ? '✅' : '⚠️'} |
