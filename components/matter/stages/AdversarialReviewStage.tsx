...
export default function AdversarialReviewStage({ matter, stage }: Props) {
  ...
  const loopCount = matter.adversarialLoopCount || 0;
  const stageData = stage.data;
  const reviewRound = stageData?.reviewRound || 1;
  const isRevisionReview = stageData?.isRevisionReview || false;
  const priorCritiquesAddressed = stageData?.priorCritiquesAddressed;
    ...
    <div className="space-y-8 animate-slide-up">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <div className="metric-card text-center">
            <div className={`text-xl font-semibold tabular-nums ${loopCount > 0 ? 'text-warning-600' : 'text-gray-900'}`}>
              {loopCount}
            </div>
            <div className="text-[11px] text-gray-500">Revision loops</div>
          </div>
          <div className="metric-card text-center">
      ...
      {/* Loopback indicator */}
      {loopCount > 0 && (
        <div className="flex items-start gap-3 p-4 bg-brand-50 border border-brand-200 rounded-lg">
          <RotateCcw className="w-4 h-4 text-brand-600 mt-0.5 shrink-0" />
          <div>
            <h3 className="text-sm font-semibold text-brand-900">Adversarial loopback engaged</h3>
            <p className="text-xs text-brand-700 mt-1 leading-relaxed">
              The adversarial reviewer identified concerns that triggered {loopCount} revision {loopCount === 1 ? 'loop' : 'loops'}. The draft was sent back to the drafting stage for revision, then re-reviewed to ensure all critiques were addressed.
            </p>
          </div>
        </div>
      )}
      ...
      {/* Prior critiques addressed (for revision reviews) */}
      {isRevisionReview && priorCritiquesAddressed && (
        <div className="card p-4">
          <h4 className="section-label mb-2">Prior critique assessment</h4>
          <p className="text-sm text-gray-600 leading-relaxed">{priorCritiquesAddressed}</p>
        </div>
      )}
      ...
      <div className="card p-4">
        ...
        <div className="grid grid-cols-3 gap-2 pt-2 border-t border-gray-100">
          {[
            { label: 'Challenge', desc: 'Tests every recommendation for weaknesses' },
            { label: 'Loopback', desc: 'Sends draft back for revision if needed (max 2×)' },
            { label: 'Verify', desc: 'Confirms fixes address original critiques' },
          ].map(({ label, desc }) => (
      ...
      {/* Escalation note for max loops */}
      {loopCount >= 2 && draftRevised && (
        <div className="flex items-start gap-3 p-4 bg-error-50 border border-error-200 rounded-lg">
          <Eye className="w-4 h-4 text-error-600 mt-0.5 shrink-0" />
          <div>
            <h3 className="text-sm font-semibold text-error-900">Max revision loops reached</h3>
            <p className="text-xs text-error-700 mt-1 leading-relaxed">
              The maximum number of adversarial revision loops (2) was reached with some concerns still unresolved. This matter has been flagged for mandatory human review in the guardrails stage.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="flex items-start gap-2.5 p-3 bg-gray-50 rounded-lg border border-gray-100">
        ...
        <p className="text-xs text-gray-500 leading-relaxed">
          <span className="font-semibold text-gray-700">Next: Quality Guardrails</span> — Automated checks verify jurisdiction compliance, citation completeness, hallucination detection, scope compliance, and ethics before deliverables are generated.
        </p>
      </div>
    </div>
  );
